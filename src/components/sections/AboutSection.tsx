  ...
  return (
    ...
    >
      ...
      <div className="max-w-6xl mx-auto relative z-10 flex flex-col items-center text-center">
        ...
        >
          {language === 'vi' ? 'Về Nguyễn Vũ Đông Quân' : 'About Nguyen Vu Dong Quan'}
        </motion.div>
        ...
        >
          {language === 'vi' ? (
            ...
            <>
              <em className="italic text-white/60">Reverse engineer</em> every problem, <br className="hidden md:block" />
              uncover the <em className="italic text-white/60">first principles</em>, and rebuild it <br className="hidden md:block" />
              in a <em className="italic text-white/60">smarter way.</em>
            </>
          )}
        </motion.h2>
        ...
        >
          ...
          <div className="lg:col-span-5 liquid-glass p-8 md:p-10 rounded-3xl border border-white/5 space-y-8 hover:border-white/10 transition-colors">
            <div>
              <h3 className="text-3xl text-white font-display mb-2">{language === 'vi' ? 'Nguyễn Vũ Đông Quân' : 'Nguyen Vu Dong Quan'}</h3>
              <p className="text-primary text-sm md:text-base font-medium tracking-wide">Gemini Certified Faculty | Google AI Specialist</p>
              <p className="text-white/40 text-sm mt-2">{language === 'vi' ? 'Đà Nẵng, Da Nang City, Vietnam' : 'Da Nang City, Vietnam'}</p>
            </div>
            
            <div className="w-full h-[1px] bg-white/5" />
            
            <div>
              <h4 className="text-xs uppercase tracking-[0.2em] text-white/30 mb-4">{language === 'vi' ? 'Năng Lực Cốt Lõi' : 'Core Capabilities'}</h4>
          ...
          <div className="lg:col-span-7 flex flex-col gap-8">
            <div className="liquid-glass p-8 md:p-10 rounded-3xl border border-white/5 hover:border-white/10 transition-colors">
              <h4 className="text-xs uppercase tracking-[0.2em] text-white/30 mb-6">{language === 'vi' ? 'Kinh Nghiệm Thực Chiến' : 'Hands-on Experience'}</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="liquid-glass p-8 rounded-3xl border border-white/5 hover:border-white/10 transition-colors">
                <h4 className="text-xs uppercase tracking-[0.2em] text-white/30 mb-5">{language === 'vi' ? 'Học Vấn' : 'Education'}</h4>
                <div className="space-y-2">
                ...
                <div className="flex-1 flex flex-col sm:flex-row gap-6 items-start">
                  ...
                  <div className="space-y-3">
                    ...
                    <a href={`${import.meta.env.BASE_URL}assets/certificates/Gemini_Certified_Faculty.pdf`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-xs font-mono uppercase tracking-wider text-primary hover:text-white transition-colors mt-2">
                      <span>{language === 'vi' ? 'Xem Bản PDF' : 'View PDF'}</span>
